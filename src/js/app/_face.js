...
const faceJson = require('../data/face.json');
const eyemouthJson = require('../data/eyemouth.json');
const fpJson = require('../data/fp.json');
...
export default class extends THREE.Mesh {
  ...
  constructor(tracker) {
    let geometry = new THREE.BufferGeometryLoader().load(faceJson)
    ...
    let material = new THREE.ShaderMaterial({
      ...
      vertexShader: require('raw-loader!./face.vert'),
      fragmentShader: require('raw-loader!./face.frag'),
  ...
  initFeaturePoints() {
    this.featurePointIndices = fpJson.map((pa, i) => {
  ...
  initEyeMouth_() {
    ...
    let fp = fpJson.map((p) => new THREE.Vector3(p[0], p[1], p[2]));
    let findFPIndex = (v) => {
    ...
    let geometry = new THREE.BufferGeometryLoader().parse(eyemouthJson)
